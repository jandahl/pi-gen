...
EOF
		...
		if [ -f "${i}-packages-nr" ]; then
			...
			if [ -n "$PACKAGES" ]; then
				on_chroot << EOF
DEBIAN_FRONTEND=noninteractive apt-get -o Acquire::Retries=3 install --no-install-recommends -y $PACKAGES
EOF
			fi
...
export ARCHIVE_FILENAME="${ARCHIVE_FILENAME:-"image_${NEW_VERSION}-${IMG_DATE}-${IMG_NAME}"}"
